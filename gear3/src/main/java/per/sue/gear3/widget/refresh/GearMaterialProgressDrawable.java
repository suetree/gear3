...
/**
 * Created by sure on 2017/10/13.
 */

public class GearMaterialProgressDrawable extends Drawable implements Animatable {
    ...
    private final Callback mCallback = new Callback() {
        public void invalidateDrawable(Drawable d) {
            GearMaterialProgressDrawable.this.invalidateSelf();
        }

        public void scheduleDrawable(Drawable d, Runnable what, long when) {
            GearMaterialProgressDrawable.this.scheduleSelf(what, when);
        }

        public void unscheduleDrawable(Drawable d, Runnable what) {
            GearMaterialProgressDrawable.this.unscheduleSelf(what);
        }
    };

    GearMaterialProgressDrawable(Context context, View parent) {
        this.mParent = parent;
    ...
    private void setupAnimators() {
        ...
        Animation animation = new Animation() {
            public void applyTransformation(float interpolatedTime, Transformation t) {
                if(GearMaterialProgressDrawable.this.mFinishing) {
                    GearMaterialProgressDrawable.this.applyFinishTranslation(interpolatedTime, ring);
                } else {
                    float minProgressArc = GearMaterialProgressDrawable.this.getMinProgressArc(ring);
                    float startingEndTrim = ring.getStartingEndTrim();
                    float startingTrim = ring.getStartingStartTrim();
                    float startingRotation = ring.getStartingRotation();
                    GearMaterialProgressDrawable.this.updateRingColor(interpolatedTime, ring);
                    float rotation;
                    float groupRotation;
                    if(interpolatedTime <= 0.5F) {
                        rotation = interpolatedTime / 0.5F;
                        groupRotation = startingTrim + (0.8F - minProgressArc) * GearMaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation(rotation);
                    ...
                    if(interpolatedTime > 0.5F) {
                        ...
                        float endTrim = startingEndTrim + rotation * GearMaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation(groupRotation);
                        ring.setEndTrim(endTrim);
                    }

                    rotation = startingRotation + 0.25F * interpolatedTime;
                    ring.setRotation(rotation);
                    groupRotation = 216.0F * interpolatedTime + 1080.0F * (GearMaterialProgressDrawable.this.mRotationCount / 5.0F);
                    GearMaterialProgressDrawable.this.setRotation(groupRotation);
                }
        ...
        animation.setAnimationListener(new Animation.AnimationListener() {
            public void onAnimationStart(Animation animation) {
                GearMaterialProgressDrawable.this.mRotationCount = 0.0F;
            }
            ...
            public void onAnimationRepeat(Animation animation) {
                ...
                if(GearMaterialProgressDrawable.this.mFinishing) {
                    GearMaterialProgressDrawable.this.mFinishing = false;
                    animation.setDuration(1332L);
                    ring.setShowArrow(false);
                } else {
                    GearMaterialProgressDrawable.this.mRotationCount = (GearMaterialProgressDrawable.this.mRotationCount + 1.0F) % 5.0F;
                }
